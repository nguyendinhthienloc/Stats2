...
Shared-fold CV selected $\lambda_{\min}=0.083342$ (CV MSE 18.4828, RMSE
4.299) and $\lambda_{1\mathrm{se}}=1.199877$ (CV MSE 20.9041, RMSE 4.572).
...
from 3166.75 to 104.76, a 30.2-fold improvement.  No Ridge slope is described
...
At $\lambda_{\min}=0.093589$, the Lasso CV MSE is 18.00 (RMSE 4.242) and
...
$\lambda_{1\mathrm{se}}=0.754567$, only \texttt{age}, \texttt{height}, and
\texttt{abdom} remain and CV MSE rises to 20.44 (RMSE 4.521).  Unlike Ridge,
...
\textbf{Lasso at $\lambda_{\min}=0.093589$}.  It has the lowest CV RMSE (4.242
versus 4.406 for OLS and 4.299 for Ridge) and reduces the model from 14 to nine
