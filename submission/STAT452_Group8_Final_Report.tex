...
For Part 1, RMSE is primary because it weights larger one-wine prediction
errors more heavily; MAE expresses typical error directly in quality-score points,
...
\subsubsection{Dataset, variables, and prediction
...
After the integrity checks in Section~\ref{data-cleaning}, the unique records were divided
...
\subsubsection{Response and predictor
...
documented in Section~\ref{data-cleaning}; EDA itself is shown on the original measurement
scale.

\subsubsection{Pairwise relationships and correlation
...
\subsubsection{Missingness, duplicates, and the locked
...
\subsubsection{Screening without deleting correlated
...
\subsubsection{Mean and OLS baselines}\label{mean-and-ols-baselines}
...
\subsubsection{Embedded Lasso selection}\label{embedded-lasso-selection}
...
\subsubsection{Cross-validation and
...
\subsubsection{Pre-holdout decision}\label{pre-holdout-decision}
...
\subsubsection{Fair model comparison}\label{fair-model-comparison}
...
\subsubsection{Diagnostics and bias--variance
...
among training, CV, and holdout RMSE in Table~\ref{tab:p5-holdout-display}. The regularized candidates have similar
...
Routine physicochemical measurements provide moderate predictive power
for red Vinho Verde quality, accounting for approximately 44\% of the
variance in expert ratings (\(R^2 \approx 0.44\), holdout
\(\text{RMSE} \approx 0.613\)). Alcohol content and volatile acidity
serve as the primary chemical signals, showing the strongest positive
and negative associations with quality, respectively. Across all
evaluated linear architectures---OLS, Ridge, Lasso, and Elastic
Net---generalization performance is practically identical on the locked
holdout set. However, Ridge is retained as the confirmatory
recommendation because \(L_2\) shrinkage stabilizes coefficient variance
in the presence of substantial multicollinearity (e.g., fixed acidity
\(\text{VIF} \approx 7.9\)) without discarding correlated chemical
dimensions.

The primary predictive bottleneck stems from the severe concentration of
sensory scores at 5 and 6, coupled with the inherent loss of
treating discrete, bounded ratings as continuous Gaussian targets. This
induces strong regression toward the mean and limits accuracy on rare,
extreme wines (scores 3, 4, and 8). Future modeling on this benchmark
should prioritize cumulative link (ordinal) models or robust non-linear
ensembles under the same strict leakage-controlled resampling protocol.
...
\(2 \times 2\) factorial analysis}, not a randomized experiment. All 1,000
...
Under equal cell weighting, students who completed preparation averaged
5.97 points higher (95\% CI \([4.09, 7.85]\)), and the standard lunch
group averaged 11.06 points higher than the free/reduced group (95\% CI
\([9.18, 12.94]\)). Both marginal nulls are rejected. The
...
points with partial \(\eta^2=0.1179\), compared with 5.97 points and
partial \(\eta^2=0.0375\) for preparation. The interaction is
practically small (partial \(\eta^2=0.0004\)) and its confidence
interval includes differences in either direction. Reporting raw points
as well as standardized effects keeps statistical significance separate
from educational importance.
...
LaTeX source integrity. Dataset discovery is not counted as a contribution.
